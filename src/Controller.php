...
class Controller {
    ...
    public static function wp2staticTrashedPostHandler(int $post_id) : void {
    ...
    public static function wp2staticEnqueueJobs( $post_id = null) : void {
        ...
        $job_types = [
            ...
            'autoJobQueueCopyUploads' => 'copy_uploads',
            'autoJobQueueDeployment' => 'deploy',
        ];

        foreach ( $job_types as $key => $job_type ) {
            if ( (int) CoreOptions::getValue( $key ) === 1 ) {
                if ($job_type === 'detect' && !is_null($post_id)) {
                    $job_meta = ['detect_last_n_day' => 2, 'post_id' => $post_id];
                    JobQueue::addJob( $job_type, $job_meta );
                } else {
    ...
    public static function wp2staticProcessJobsNow() : void {
        check_admin_referer( 'wp2static-process-jobs-now' );
    ...
    */
    public static function wp2staticProcessQueue(bool $useCli = null) : void {
        if (defined( 'WP_CLI' ) || $useCli === false) {
            set_time_limit(0);
            global $wpdb;
            ...
            if (!empty($jobs)) {
                if ($useCli === false) {
                    WsLog::l('Processing queue via normal flow.');
                } else {
                    WsLog::l('Processing queue via WP-CLI.');
                }
            }
            foreach ( $jobs as $job ) {
                if ($job->job_meta) {
                    $job->job_meta = json_decode($job->job_meta, true);
                }
                try {
                    JobProcessor::processJob($job);
                } catch (\Throwable $e) {
                    WsLog::l('Error processing job: ' . $e->getMessage());
                }
            }
        } else if ($useCli === null) {
            self::wp2staticProcessQueue(CoreOptions::getValue( 'useWpCil' ) ?? false);
        } else if ($useCli === true) {
            try {
                // get php executable
                $php_version = phpversion();
                $php_version = substr($php_version, 0, 3);
                $php_executable = shell_exec(
                    sprintf("which %s", escapeshellarg('php' . $php_version))
                )? 'php' . $php_version : 'php';

                $wp_executable = plugin_dir_path( __DIR__ ) . 'bin/wp-cli.phar';

                if (!file_exists($wp_executable)) {
                    WsLog::l('WP-CLI is not found.');
                    throw new WP2StaticException(
                        'wp-cli is not installed. Please install wp-cli first.'
                    );
                }

                $host = $_SERVER['HTTP_HOST'];
                $command = <<<EOT
                nohup >/dev/null 2>&1 ${php_executable} ${wp_executable} --url="${host}" wp2static process_queue &
                EOT;
                exec($command, $output, $return_var);
            } catch (\Exception $e) {
                WsLog::l('Error while trying to process queue: ' . $e->getMessage());
            }
        }
    }
    ...
    public static function wp2staticDetect() : void {
        check_ajax_referer( 'wp2static-run-page', 'security' );

        JobQueue::addJob( 'detect' );
        self::wp2staticProcessQueueAdminPost();

        wp_die();
    }

    public static function wp2staticCrawlOnly() : void {
        check_ajax_referer( 'wp2static-run-page', 'security' );

        JobQueue::addJob( 'crawl' );
        self::wp2staticProcessQueueAdminPost();

        wp_die();
    }

    public static function wp2staticCopyUploads()
    {
        check_ajax_referer( 'wp2static-run-page', 'security' );

        JobQueue::addJob( 'copy_uploads' );
        self::wp2staticProcessQueueAdminPost();
    ...
    public static function wp2staticPostProcess() : void {
        check_ajax_referer( 'wp2static-run-page', 'security' );

        JobQueue::addJob( 'post_process' );
        self::wp2staticProcessQueueAdminPost();
